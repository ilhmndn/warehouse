	...
	total_harga = (total_harga + (parseInt(harga[i].qty) * parseInt(harga[i].harga)))
	 ...
	 frappe.call({
		...
		callback:function (r){
			...
			// if (r.message.status == 'Tidak Aktif'){
			// 	frappe.throw('Mohon Maaf status Perusahaan Sudah tidak aktif, silahkan memperbarui status terlebih dahulu')
			// 	frm.set_value('nama_supplier', '')
			// }
		}
	});
  });
